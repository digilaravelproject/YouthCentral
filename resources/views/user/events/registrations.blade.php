@extends('layouts.user_type.auth') {{-- Assuming users use the same auth layout --}}
        ...
        <div class="col-12">
            <div class="card mb-4">
                <div class="card-header pb-0 d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('user.my-events.index') }}" class="btn btn-sm btn-outline-secondary mb-0">
                            ...
                            <table class="table align-items-center mb-0">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-secondary opacity-7"></th> {{-- Actions column --}}
                                    </tr>
                                </thead>
                                <tbody>
                                        ...
                                        <tr>
                                            <td>
                                                <div class="d-flex px-2 py-1">
                                                    {{-- You might want an icon/avatar here eventually --}}
                                                    <div class="d-flex flex-column justify-content-center">
                                                        <h6 class="mb-0 text-sm">{{ $registration->first_name }} {{ $registration->last_name }}</h6>
                                                        {{-- Optionally show user link if registered user --}}
                                                        {{-- @if($registration->user)
                                                            <p class="text-xs text-secondary mb-0">Registered User: {{ $registration->user->name }}</p>
                                                        @endif --}}
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                                ...
                                                {{-- @if($registration->phone)
                                                    <p class="text-xs text-secondary mb-0">{{ $registration->phone }}</p>
                                                @endif --}}
                                            </td>
                                            ...
                                            <td class="align-middle text-center">
                                                {{-- General events are marked 'completed' upon registration (no payment needed) --}}
                                                <span class="badge badge-sm bg-gradient-success">
                                                    Completed
                                                </span>
                                            </td>
                                            <td class="align-middle">
                                                <a href="{{ route('user.my-events.registrations.show', [$event->id, $registration->id]) }}" 
