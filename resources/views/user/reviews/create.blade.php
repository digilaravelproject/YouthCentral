            ...
            <div class="card mb-4">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <a href="{{ url()->previous() }}" class="btn btn-outline-secondary btn-sm">Go Back</a>
                    </div>
                </div>
                <div class="card-body">
                    @if(session('success'))
                    ...
                    <div class="business-info-card mb-4">
                        <div class="card bg-gradient-info">
                            <div class="card-body p-3">
                                <div class="row">
                                    <div class="col-12">
                                        <h5 class="text-white mb-0">{{ $business->name }}</h5>
                                        <p class="text-white text-sm mb-0 opacity-8">{{ $business->subcategory->name }} in {{ $business->area->name }}, {{ $business->area->city->name }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <form method="POST" action="{{ route('user.reviews.store') }}">
                        ...
                        <div class="form-group mb-4">
                            ...
                            <textarea class="form-control @error('comment') is-invalid @enderror" id="comment" name="comment" rows="5" placeholder="Share your experience with this business...">{{ old('comment') }}</textarea>
                            @error('comment')
                        ...
                        <div class="form-group">
                            <div class="alert alert-info text-white text-sm" role="alert">
                                ...
                                Your review will be visible after it is approved by an administrator. You can edit your review until it is approved.
                            </div>
                        </div>
                        
                        <div class="d-flex justify-content-end mt-4">
                            <a href="{{ url()->previous() }}" class="btn btn-light me-2">Cancel</a>
                            <button type="submit" class="btn bg-gradient-primary">Submit Review</button>
                        </div>
                    </form>
                </div>
            </div>
