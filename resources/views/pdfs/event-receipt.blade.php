    ...
    <div class="receipt">
        <div class="header">
            @php
             ...
             @endphp
            <img src="{{$logoSrc}}" class="logo pdf_logo" alt="{{ $company_name }}" >
            {{-- <img src="{{ public_path('assets/img/logos/logo.png') }}" class="logo" alt="{{ $company_name }}"> --}}
        ...
        <div class="row">
            ...
            <div class="customer-info">
                ...
                <div class="info-row">{{ $registration->address }}</div>
        ...
        <div class="event-details">
            ...
            <div class="info-row">
                <span class="info-label">Event Name:</span> {{ $event->title }}
            </div>
            <div class="info-row">
                <span class="info-label">Date:</span> {{ $event->start_date->format('D, M d, Y') }}
            </div>
            <div class="info-row">
                <span class="info-label">Time:</span> {{ $event->start_date->format('h:i A') }} - {{ $event->end_date->format('h:i A') }}
            </div>
            <div class="info-row">
                <span class="info-label">Venue:</span> {{ $event->venue }}
            </div>
            <div class="info-row">
                <span class="info-label">Sport/Event:</span> {{ $registration->sport_event }}
            </div>
            @if($registration->age_category)
            <div class="info-row">
                <span class="info-label">School Type:</span> {{ $registration->age_category }}
            </div>
            @endif
        </div>
        ...
        <table>
            ...
            <tbody>
                <tr>
                    ...
                    <td class="amount">{{ $currency_symbol }}{{ number_format($registration->amount_paid ?? $registration->amount, 2) }}</td>
                </tr>
                <tr class="total-row">
                    <td>Total</td>
                    <td class="amount">{{ $currency_symbol }}{{ number_format($registration->amount_paid ?? $registration->amount, 2) }}</td>
        ...
        <div class="payment-info">
            ...
            <div class="info-row">
                <span class="info-label">Payment Status:</span> Paid
            </div>
            <div class="info-row">
                <span class="info-label">Payment Date:</span> {{ $registration->updated_at->format('M d, Y h:i A') }}
            </div>
            <div class="info-row">
                <span class="info-label">Payment Method:</span> Razorpay
            </div>
            <div class="info-row">
                <span class="info-label">Transaction ID:</span> {{ $registration->razorpay_payment_id }}
            </div>
        </div>
