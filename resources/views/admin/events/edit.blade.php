        ...
        <div class="col-12">
            <div class="card mb-4">
                <div class="card-header pb-0 d-flex justify-content-between align-items-center">
                    <h6 class="mb-0">Edit Event: <span class="text-info">{{ $event->title }}</span></h6>
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.events.update', $event->slug) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="d-flex flex-wrap current-event-banners">
                            ...
                            <style>
                                .current-event-banners { border-bottom: 2px solid #f1f1f1; padding-bottom: .5rem; }
                            </style>
                        </div>
                        @endif
                        ...
                        <div class="mb-3">
                            <p class="text-sm mb-1">Current Gallery Images:</p>
                            <div class="d-flex flex-wrap" id="existing-gallery">
                                @foreach($event->additional_images as $img)
                                    <div class="position-relative me-2 mb-2 existing-img-wrapper" data-path="{{ $img }}">
                                        <img src="{{ asset('storage/' . $img) }}" height="80" class="rounded">
                                        <span class="remove-existing position-absolute top-0 start-100 translate-middle p-1 bg-danger text-white rounded-circle cursor-pointer" style="font-size:1rem;line-height:1;">&times;</span>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control form-control-sm" id="long_description" name="long_description" rows="6">{{ old('long_description', $event->long_description) }}</textarea>
    ...
    if (bannerInput) {
        bannerInput.addEventListener('change', function () {
            ...
            for (const file of bannerInput.files) {
                ...
                if (!valid) {
                    if (!typeValid) {
                        showError(bannerInput, 'Only .webp format is allowed for banner images.');
                    } else if (!sizeValid) {
                        showError(bannerInput, 'Each banner image must be less than 2MB.');
                    }
                    bannerInput.value = ''; // Clear files if invalid found
                    break;
                }
            }
        });
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        //         // Don't null the value here on edit, let the controller handle it if category changes
        //     } else {
