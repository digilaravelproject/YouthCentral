                ...
                <div class="card-body p-3">
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                <h6 class="mb-0 text-sm">Import Areas from Excel</h6>
                                <p class="text-xs text-muted mb-0">Upload an XLSX file to populate areas data</p>
                            </div>
                        </div>
                        <div class="d-flex align-items-center flex-wrap gap-2">
                            <!-- Import Form -->
                            <form action="{{ route('admin.areas.import') }}" method="POST" enctype="multipart/form-data" class="d-flex align-items-center mb-0">
                                @csrf
                            ...
                            <a href="{{ route('admin.areas.sample') }}" class="btn btn-outline-primary btn-sm mb-0">Download Sample XLSX</a>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            <!-- Bulk Delete Form -->
                            <form id="bulk-delete-form" action="{{ route('admin.areas.bulk-delete') }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete the selected areas?')">
                                @csrf
                                <input type="hidden" name="ids" id="bulk-delete-ids">
                                <button type="submit" id="bulk-delete-btn" class="btn btn-danger btn-sm mb-0" disabled>Delete Selected</button>
                            </form>
                            
                            <a href="{{ route('admin.areas.create') }}" class="btn btn-primary btn-sm mb-0">Add New</a>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="table align-items-center mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <p class="text-xs font-weight-bold mb-0">{{ $area->city->name }}</p>
                                    </td>
                                    <td>
                                        <p class="text-xs font-weight-bold mb-0">{{ $area->city->state->name }}</p>
                                    </td>
                                    <td class="align-middle">
                                        <a href="{{ route('admin.areas.edit', $area) }}" class="text-secondary font-weight-bold text-xs me-3" data-toggle="tooltip" data-original-title="Edit area">
                                            Edit
                                        </a>
