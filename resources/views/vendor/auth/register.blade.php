...
<html lang="en-US">
  <head>
    ...
    <title>Youth Central - Vendor Registration</title>
    <!-- Google Font(s) -->
    ...
    <div class="body-wrapper">
      ...
      <header class="header">
        ...
        <div class="user-buttons">
          <a href="{{ route('user.register') }}" class="add-listing">Customer Registration</a>
        </div>
        <!-- End User Buttons -->
      </header>
      <!-- End header (topbar) -->
    			...
    			<div class="col-sm-12">
    				<div class="panel panel-form">
    					<div class="panel-heading">
    						<div class="row">
    							<div class="col-xs-6">
    								<a href="{{ route('vendor.login') }}" id="login-form-link">Login</a>
    							</div>
    							<div class="col-xs-6 text-right">
    								<a href="#" class="active" id="register-form-link">Register as Vendor</a>
    							</div>
    						</div>
    						<hr>
    					</div>
    					<div class="panel-body">
    						<div class="row">
    							<div class="col-lg-12">
                  <form role="form text-left" method="POST" action="{{ route('vendor.register') }}">
                    @csrf
                    <div class="section-title">Personal Information</div>
                    <div class="form-group">
                      <input type="text" class="form-control @error('name') is-invalid @enderror" placeholder="Owner Name" aria-label="Name" aria-describedby="name-addon" name="name" value="{{ old('name') }}" required>
                      @error('name')
                    ...
                    <div class="form-group">
                      <input type="text" class="form-control @error('business_name') is-invalid @enderror" placeholder="Business Name" aria-label="Business Name" aria-describedby="business-name-addon" name="business_name" value="{{ old('business_name') }}" required>
                      @error('business_name')
                        <div class="invalid-feedback">{{ $message }}</div>
                      @enderror
                    </div>
                    <div class="form-group">
                      <input type="text" class="form-control @error('business_address') is-invalid @enderror" placeholder="Business Address" aria-label="Business Address" aria-describedby="business-address-addon" name="business_address" value="{{ old('business_address') }}" required>
                      @error('business_address')
                        <div class="invalid-feedback">{{ $message }}</div>
                      @enderror
                    </div>
                    <div class="form-group">
                      <input type="text" class="form-control @error('gst_number') is-invalid @enderror" placeholder="GST Number (Optional)" aria-label="GST Number" aria-describedby="gst-number-addon" name="gst_number" value="{{ old('gst_number') }}">
                      @error('gst_number')
                        <div class="invalid-feedback">{{ $message }}</div>
                      @enderror
                    </div>
