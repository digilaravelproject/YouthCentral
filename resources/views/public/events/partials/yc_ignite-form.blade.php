<form action="{{ route('yc-ignite.store') }}" method="POST" id="event-registration-form">
    @csrf
    ...
    <div class="form-group mb-3">
        ...
        <input type="text" class="form-control @error('first_name') is-invalid @enderror" id="first_name"
            name="first_name" value="{{ old('first_name', $user ? $user->name : '') }}" required>
        @error('first_name')
    ...
    <div class="form-group mb-3">
        ...
        <input type="text" class="form-control @error('mobile') is-invalid @enderror" id="mobile" name="mobile"
            value="{{ old('mobile', $user ? $user->phone : '') }}" required>
        @error('mobile')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
    ...
    {{-- <div class="form-group mb-3">
        ...
        <select class="form-control @error('age_category') is-invalid @enderror" id="age_category" name="age_category" required>
    ...
    <div class="form-group mb-3">
        ...
        <select class="form-control @error('sport_event') is-invalid @enderror" id="sport_event" name="sport_event"
            required>
    ...
    @if ($event->registration_amount > 0)
        <div class="alert alert-info mb-3">
            <i class="fas fa-info-circle me-2"></i> <strong>Note:</strong> Government school students get 20% cashback
            after verification of school ID or related document.
        </div>
    @endif
            ...
            <div class="card border">
                <div class="card-body">
                    <div class="form-check mb-3">
                        <input class="form-check-input @error('parent_consent') is-invalid @enderror" type="checkbox"
                            id="parent_consent" name="parent_consent" required
                            {{ old('parent_consent') ? 'checked' : '' }}>
                        <label class="form-check-label" for="parent_consent" style="font-size:12px;">
                            I am the parent/guardian of the student registering for YC IGNITE 2025. I give my consent for
                            their participation in this online event and agree that Youth Central may collect and
                            securely use the required student information for registration and event communication.
                        </label>
                    ...
                    <div class="form-check mb-2">
                        <input class="form-check-input @error('terms_conditions') is-invalid @enderror"
                            type="checkbox" id="terms_conditions" name="terms_conditions" required
                            {{ old('terms_conditions') ? 'checked' : '' }}>
                        <label class="form-check-label" for="terms_conditions" style="font-size:12px;">
                            ...
                            I have read and understood the <a href="{{ route('events.parent-guardian-consent') }}"
                                target="_blank">Terms & Conditions of YC SPARK 2025</a> and agree on behalf of the
                            student.
                        </label>
